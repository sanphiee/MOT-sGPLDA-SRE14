function [eer, dcf08, dcf10, dcf14] = compute_eer(scores, labels, showfig)
...
FN = cumsum( x == 1 ) / (sum( x == 1 ) + eps); %©��  # misses(thresh=t) / # target trials
TN = cumsum( x == 0 ) / (sum( x == 0 ) + eps); %
FP = 1 - TN;     %�龯 # false alarms(thresh=t) / # non-target trials
TP = 1 - FN;
